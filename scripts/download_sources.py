...
def download_and_extract_hebrew():
...
def download_and_extract_greek():
    ...
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        for file in zip_ref.namelist():
            ...
            if (file.endswith('.xml') and 
                '/no-accents/' in file and 
                '/BYZ/' not in file and
                os.path.basename(file) != 'byz.xml'):
...
def download_and_extract_strongs():
...
def generate_manifest():
    ...
    # Save manifest
    with open('data/sources/manifest.json', 'w') as f:
...
def download_translations():
    """Download public domain Bible translations."""
    print("Downloading Bible translations...")
    
    # Using Berean Bible project which has many public domain translations
    # World English Bible
    print("  Downloading World English Bible (WEB)...")
    web_url = "https://raw.githubusercontent.com/BibleNLP/ebible/main/corpus/WEB.txt"
    web_path = os.path.join('data/sources/translations', 'web.txt')
    try:
        urllib.request.urlretrieve(web_url, web_path)
    except Exception as e:
        print(f"    Failed to download WEB: {e}")
    
    # King James Version from different source
    print("  Downloading King James Version (KJV)...")
    kjv_url = "https://www.o-bible.com/download/kjv.gz"
    kjv_gz_path = os.path.join('data/sources/translations', 'kjv.gz')
    kjv_path = os.path.join('data/sources/translations', 'kjv.txt')
    try:
        urllib.request.urlretrieve(kjv_url, kjv_gz_path)
        # Decompress
        import gzip
        with gzip.open(kjv_gz_path, 'rb') as f_in:
            with open(kjv_path, 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        os.remove(kjv_gz_path)
    except Exception as e:
        print(f"    Failed to download KJV: {e}")
    
    # For now, let's use a simpler approach with USFM files from eBible.org
    print("  Note: For production use, consider downloading USFM files from eBible.org")
    print("        which has properly formatted public domain translations")
    
    # Create placeholder files for testing
    translations_info = {
        "web_info.json": json.dumps({"name": "World English Bible", "license": "Public Domain", "url": "https://ebible.org/web/"}, indent=2),
        "kjv_info.json": json.dumps({"name": "King James Version", "license": "Public Domain", "url": "https://ebible.org/kjv/"}, indent=2),
        "asv_info.json": json.dumps({"name": "American Standard Version", "license": "Public Domain", "url": "https://ebible.org/asv/"}, indent=2),
        "ylt_info.json": json.dumps({"name": "Young's Literal Translation", "license": "Public Domain", "url": "https://ebible.org/ylt/"}, indent=2)
    }
    
    for filename, content in translations_info.items():
        with open(os.path.join('data/sources/translations', filename), 'w') as f:
            f.write(content)
    
    print(f"  Created {len(os.listdir('data/sources/translations'))} translation info files")
