...
from .semantic_concordance import ConceptDefinition, SemanticConcordance, SemanticMatch
...
class ConceptMapper:
        ...
        for i, concept_data in enumerate(concepts_to_process, 1):
            ...
            try:
                stat = self.process_concept(
                    concept_data, max_semantic_results=max_semantic_per_concept, validate_semantic=validate_semantic
                )
                stats[name] = stat

                # Show progress
                logger.info(
                    f"   ✓ {name}: {stat.total_matches} matches "
    ...
    def process_concept(
        self, concept_data: Dict, max_semantic_results: int = 100, validate_semantic: bool = True
    ) -> ConceptMappingStats:
        ...
        matches = self.semantic_concordance.build_semantic_concordance(
            concept, max_semantic_results=max_semantic_results, validate_semantic=validate_semantic
        )
    ...
    def _calculate_stats(
        self, concept_name: str, matches: List[SemanticMatch], processing_time: float
    ) -> ConceptMappingStats:
