...
class SemanticAnalysisPipeline:
    ...
    def map_concept_to_verses(self, concept: ConceptDefinition) -> ConceptMappingResult:
        ...
        try:
            ...
            if result.traditional_matches:
                validated, false_positives = self._validate_traditional_matches(
                    concept, result.traditional_matches
                )
            ...
            additional_matches = self._comprehensive_verse_scan(
                concept, exclude_verses=set(result.traditional_matches)
            )
            ...
            # Log summary
            total_matches = (len(result.llm_validated_matches) + 
                           len(result.llm_discovered_matches))
    ...
    def _validate_traditional_matches(self,
                                    concept: ConceptDefinition,
                                    verse_ids: List[str]) -> Tuple[List[str], List[str]]:
        ...
        results = self.analyzer.batch_analyze_verses(
            verses, concept.name, concept.description
        )
        ...
        for result in results:
            ...
            if (result.relevance_score >= 0.5 and 
    ...
    def _comprehensive_verse_scan(self,
                                concept: ConceptDefinition,
                                exclude_verses: set = None) -> List[str]:
        ...
        if all_verses:
            ...
            results = self.analyzer.batch_analyze_verses(
                all_verses, concept.name, concept.description
            )
            
            # Keep verses with high relevance and consensus
            for result in results:
                if (result.relevance_score >= 0.6 and  # Higher threshold for discovery
                    result.consensus_reached and
                    not result.error):
