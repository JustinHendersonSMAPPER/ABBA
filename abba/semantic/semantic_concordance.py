...
from .strongs_concordance import StrongsConcordance, ConceptDefinition, ConcordanceMatch
...
class SemanticConcordance:
    ...
    def build_semantic_concordance(self, concept: ConceptDefinition, 
                                 max_semantic_results: int = 100,
                                 validate_semantic: bool = True) -> List[SemanticMatch]:
        """
        ...
        if hebrew_prototype is not None or greek_prototype is not None:
            semantic_candidates = self._find_semantic_matches(
                ...
                exclude_verse_ids=lexical_verse_ids
            )
    ...
    def _build_concept_prototypes(self, seed_matches: List[ConcordanceMatch]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    def _find_semantic_matches(self, hebrew_prototype: Optional[np.ndarray], 
                              greek_prototype: Optional[np.ndarray],
                              max_results: int,
                              exclude_verse_ids: Set[str]) -> List[Dict]:
    ...
    def _validate_semantic_matches(self, concept: ConceptDefinition, 
                                  candidates: List[Dict]) -> List[SemanticMatch]:
    ...
    def _validate_with_ollama(self, concept: ConceptDefinition, 
                             verse_data: Dict) -> ValidationResult:
    ...
    def _rank_combined_results(self, matches: List[SemanticMatch]) -> List[SemanticMatch]:
        ...
        for match in matches:
            if match.is_semantic_only:
                ...
                match.confidence = (
                    0.3 * match.semantic_score +  # Embedding similarity
                    0.7 * match.ollama_confidence  # LLM validation
                )
        ...
        return sorted(
            matches,
            key=lambda m: (-m.confidence, m.book, m.chapter, m.verse)
        )
    
    def generate_semantic_report(self, concept: ConceptDefinition,
                                matches: List[SemanticMatch]) -> str:
