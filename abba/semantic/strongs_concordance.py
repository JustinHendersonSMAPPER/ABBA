...
class StrongsConcordance:
    ...
    def _validate_database_schema(self):
        ...
        with self.db_manager.get_connection() as conn:
            ...
            if missing:
                raise ValueError(
                    f"Database schema incomplete. Missing columns: {missing}. "
                    "Run fix_stepbible_schema.py first."
                )
    
    def define_concept(self, name: str, primary_strongs: List[str], 
                      extended_strongs: Optional[List[str]] = None,
                      phrase_patterns: Optional[List[Dict]] = None) -> ConceptDefinition:
        """
    ...
    def _validate_strongs_numbers(self, strongs_list: List[str]):
        ...
        with self.db_manager.get_connection() as conn:
            ...
            for strongs in strongs_list:
                ...
                cursor.execute(
                    f"SELECT COUNT(*) FROM stepbible_verses WHERE {placeholders}",
                    variants
                )
    ...
    def build_concordance(self, concept: ConceptDefinition) -> List[ConcordanceMatch]:
        ...
        matches.extend(self._find_strongs_matches(
            concept.primary_strongs, 
            match_type='primary',
            confidence=1.0
        ))
        ...
        if concept.phrase_patterns:
            matches.extend(self._find_phrase_matches(
                concept.phrase_patterns,
                confidence=0.9
            ))
    ...
    def _find_strongs_matches(self, strongs_list: List[str], 
                             match_type: str, confidence: float) -> List[ConcordanceMatch]:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            for strongs in strongs_list:
                ...
                for row in cursor.fetchall():
                    ...
                    matches.append(ConcordanceMatch(
                        verse_id=verse_id,
                        book=row[1],
                        chapter=row[2],
                        verse=row[3],
    ...
    def _find_phrase_matches(self, phrase_patterns: List[Dict], 
                            confidence: float) -> List[ConcordanceMatch]:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            for lemma, original_strongs in lemma_map.items():
                ...
                if variant_strongs:
                    ...
                    variant_matches = self._find_strongs_matches(
                        variant_strongs,
                        match_type='lemma',
                        confidence=0.7
                    )
    ...
    def generate_report(self, concept: ConceptDefinition, 
                       matches: List[ConcordanceMatch]) -> str:
