...
class ParallelStepBibleImporter:
    ...
    def __init__(self, stepbible_dir: Path, dest_db_path: Path, max_workers: Optional[int] = None):
    ...
    def parse_file_parallel(  # noqa: C901
        ...
        for i, raw_line in enumerate(all_lines):
            ...
            if (
                ...
                and "STEPBible.org" not in line
                and "Data created by" not in line
                and "licence allows" not in line
                and "." in line
                and "#" in line
                and "=" in line
                and "\t" in line
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            if show_progress:
                pbar = tqdm(total=len(chunks), desc=f"    {filename}", unit="chunks")
    ...
    def _parse_chunk(job: StepBibleJob, chunk_id: int) -> StepBibleResult:  # noqa: C901
        ...
        try:
            ...
            duration = time.time() - start_time
            return StepBibleResult(
                filename=job.filename, chunk_id=chunk_id, success=False, words_parsed=0, duration=duration, error=str(e)
            )
    ...
    def _insert_word_batch(self, word_data: List[Dict[str, Any]]):
        ...
        with sqlite3.connect(self.dest_db_path) as conn:
            ...
            for word in word_data:
                ...
                hash_data = (
                    f"{word.get('original_word', '')}|{word.get('strongs_raw', '')}|{word.get('morphology', '')}"
                )
