...
class HashValidator:
    ...
    def validate_translation_import(
        ...
        try:
                    ...
                    for row in source_cursor:
                        ...
                        if not dest_row:
                            missing_verses.append(
                                f"{row['book_id']}:{row['chapter']}:{row['verse']}"
                            )
                        else:
                            # Verify hash matches
            ...
            if dest_count != verse_count:
                return False, f"Count mismatch: expected {verse_count}, found {dest_count}", {
                    'expected_count': verse_count,
                    'actual_count': dest_count
                }
            
            return True, f"Successfully validated {verse_count} verses", {
    ...
    def calculate_aggregate_checksum(
        self,
        db_path: str,
        translation_id: str
    ...
    def validate_embeddings(
        ...
        try:
            ...
            if not results['ids']:
                return False, f"No embeddings found for {translation_id}", {
            ...
            for i, verse_id in enumerate(results['ids']):
                ...
                if source_hash and source_hash != expected_hashes[verse_id]:
                    hash_mismatches.append({
                        'verse_id': verse_id,
                        'expected': expected_hashes[verse_id],
                        'actual': source_hash
                    })
            ...
            missing_embeddings = [
                vid for vid in expected_hashes.keys() 
                if vid not in embedded_ids
            ]
            ...
            return True, f"Successfully validated {len(results['ids'])} embeddings", {
    ...
    def quick_validate(
        ...
        """
        source_checksum, source_count = self.calculate_aggregate_checksum(
            source_db_path, translation_id
        )
        
        dest_checksum, dest_count = self.calculate_aggregate_checksum(
            dest_db_path, translation_id
        )
