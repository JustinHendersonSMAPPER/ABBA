...
class OperationManager:
    ...
    def complete_job(
        ...
        if validation_handler:
            try:
                is_valid, message, details = validation_handler(db_manager, chroma_manager, job_name, validation_params)

                validation_result = {"valid": is_valid, "message": message, "timestamp": datetime.now().isoformat()}
    ...
    def _validate_translation_import(
        ...
        is_valid, message, details = self.validator.validate_translation_import(
            translation_id, str(self.source_db_path), dest_db_path
        )
        ...
        if is_valid and validation_params and validation_params.get("use_checksum", True):
            quick_valid, quick_msg = self.validator.quick_validate(
                str(self.source_db_path), dest_db_path, translation_id
            )
    ...
    def _validate_stepbible_import(
        ...
        with db_manager.get_connection() as conn:
            ...
            cursor.execute("SELECT COUNT(*) FROM stepbible_verses WHERE source_file = ?", (file_name,))
            ...
            if expected_counts.get("verses"):
                ...
                if actual_verses < expected * 0.95:  # 5% tolerance
                    return (
                        False,
                        f"Expected ~{expected} verses, found {actual_verses}",
                        {"file": file_name, "expected": expected, "actual": actual_verses},
                    )
            ...
            return (
                True,
                f"Imported {actual_verses:,} verses from {file_name}",
                {"file": file_name, "verse_count": actual_verses},
            )
    ...
    def _validate_verse_embeddings(
        ...
        is_valid, message, details = self.validator.validate_embeddings(
            translation_id, str(db_manager.db_path), chroma_manager
        )
    ...
    def _validate_word_embeddings(
        ...
        try:
            ...
            if actual_count < expected_word_count * 0.8:  # Allow 20% variance for deduplication
                return (
                    False,
                    f"Expected ~{expected_word_count:,} word embeddings, found {actual_count:,}",
                    {
            ...
            return (
                True,
                f"Successfully embedded {actual_count:,} words",
                {"word_count": actual_count, "expected_count": expected_word_count},
            )
    ...
    def handle_interrupted_operations(self) -> List[str]:
        ...
        for operation, job in interrupted:
            warnings.append(f"⚠️  {operation}/{job} was interrupted and will be cleaned up on next run")
