...
class ContextBuilder:
    ...
    def build_verse_context(self, translation_id: str, book_id: int, chapter: int, verse: int) -> str:
            ...
            with self.db.get_connection() as conn:
                ...
                if word_data.get("morphology_code"):
                    morph_desc = self._get_morphology_description(
                        cursor, word_data["morphology_code"], word_data.get("language", "greek")
                    )
                    if morph_desc:
                        context_parts.append(f"Form: {morph_desc}")

                # Add usage context (sample verses where it appears)
                usage_samples = self._get_usage_samples(cursor, word_data["strongs_primary"], limit=3)
                if usage_samples:
                    context_parts.append(f"Used in: {', '.join(usage_samples)}")

        return " | ".join(context_parts)

    def build_batch_contexts(self, items: List[Dict[str, Any]], context_type: str = "verse") -> List[str]:
        ...
        for item in items:
            if context_type == "verse":
                context = self.build_verse_context(
                    item["translation_id"], item["book_id"], item["chapter"], item["verse"]
                )
    ...
    def _extract_key_terms(self, cursor, words: List[tuple]) -> str:
        ...
        for word in words[:15]:  # Limit to first 15 words
            ...
            if (
                not strongs
                or strongs in seen_strongs
                or strongs.startswith("H9")  # Hebrew particles
                or strongs in ["G3588", "G2532", "G1161"]
            ):  # Common Greek articles/conjunctions
                continue
    ...
    def _get_morphology_description(self, cursor, morph_code: str, language: str) -> Optional[str]:
    ...
    def _get_usage_samples(self, cursor, strongs_number: str, limit: int = 3) -> List[str]:
