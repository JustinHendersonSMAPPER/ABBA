...
class ChromaManager:
    ...
    def __init__(self, persist_path: str = "bible_data/vectors"):
        ...
        try:
            self.client = chromadb.PersistentClient(
                ...
                settings=Settings(
                    anonymized_telemetry=False, allow_reset=True, persist_directory=str(self.persist_path)
                ),
            )
    ...
    def generate_verse_id(self, translation_id: str, book_id: int, chapter: int, verse: int) -> str:
    ...
    def parse_verse_id(self, verse_id: str) -> Dict[str, Any]:
        ...
        return {"translation_id": parts[0], "book_id": int(parts[1]), "chapter": int(parts[2]), "verse": int(parts[3])}
    ...
    def _format_search_results(self, raw_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, (id_, similarity, metadata) in enumerate(zip(ids, similarities, metadatas)):
            result = {"id": id_, "similarity": similarity, "metadata": metadata, "rank": i + 1}
