...
class PostImportValidator:
    ...
    def validate_all_translations(self) -> ValidationSummary:
        ...
        with sqlite3.connect(self.abba_db_path) as abba_conn, sqlite3.connect(self.source_db_path) as source_conn:
            ...
            for trans_id in translation_ids:
                ...
                # Check verse counts
                results.extend(self._validate_verse_counts(trans_id, abba_conn, source_conn))

                # Check book coverage
                results.extend(self._validate_book_coverage(trans_id, abba_conn, source_conn))

                # Check canon association
                results.extend(self._validate_canon_association(trans_id, abba_conn, source_conn))

                # Check data integrity
                results.extend(self._validate_data_integrity(trans_id, abba_conn, source_conn))
    ...
    def _validate_verse_counts(
        self, trans_id: str, conn: sqlite3.Connection, source_conn: sqlite3.Connection
    ...
    def _validate_canon_association(
        self, trans_id: str, conn: sqlite3.Connection, source_conn: sqlite3.Connection
    ) -> List[ValidationResult]:
        """Validate canon detection and association."""
        results = []

        # Detect canon using the shared source connection
