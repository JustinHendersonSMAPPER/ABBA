        ...
        with sqlite3.connect(db_path) as conn:
            ...
            if source_db_path.exists():
                for trans_id in translation_ids:
                    ...
                    cursor.execute(
                        "UPDATE translations SET canon = ? WHERE id = ?",
                        (canon_enum.value, trans_id)
                    )
...
def add_stepbible_verses_table(db_path: Path) -> bool:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            # Create indexes
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_stepbible_verses_reference ON stepbible_verses(book, chapter, verse)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_stepbible_verses_source ON stepbible_verses(source_file)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_stepbible_verses_strongs ON stepbible_verses(strongs_primary)")
