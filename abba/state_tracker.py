...
class StateTracker:
    ...
    def _migrate_old_format(self, old_data: Dict) -> Dict:
        ...
        for category, operations in old_data.items():
            ...
            new_state["operations"][op_name] = OperationState(
                status=OperationStatus.IN_PROGRESS, started_at=datetime.now().isoformat(), jobs={}
            )

            for job_name, job_data in operations.items():
                if isinstance(job_data, dict):
                    new_state["operations"][op_name].jobs[job_name] = JobState(
                        status=OperationStatus(job_data.get("status", "never_started")), progress=job_data
                    )

        return new_state

    def start_operation(self, operation_name: str, metadata: Optional[Dict[str, Any]] = None) -> OperationState:
        ...
        op_state = OperationState(
            status=OperationStatus.IN_PROGRESS, started_at=datetime.now().isoformat(), metadata=metadata or {}
        )
    ...
    def start_job(self, operation_name: str, job_name: str, metadata: Optional[Dict[str, Any]] = None) -> JobState:
    ...
    def update_job_progress(self, operation_name: str, job_name: str, progress: Dict[str, Any]):
    ...
    def complete_job(
        ...
        all_complete = all(j.status == OperationStatus.COMPLETED for j in op_state.jobs.values())
    ...
    def get_job_state(self, operation_name: str, job_name: str) -> Optional[JobState]:
    ...
    def should_cleanup_job(self, operation_name: str, job_name: str) -> bool:
        ...
        return job_state.status in [OperationStatus.INTERRUPTED, OperationStatus.FAILED]
    ...
    def reset_job(self, operation_name: str, job_name: str):
    ...
    def get_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for op_name, op_state in self.state.get("operations", {}).items():
            if isinstance(op_state, OperationState):
                ...
                for job_name, job_state in op_state.jobs.items():
                    if isinstance(job_state, JobState):
                        op_summary["jobs"][job_name] = {"status": job_state.status.value}
